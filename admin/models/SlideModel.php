<?php 
	trait SlideModel{
		...
		public function modelUpdate(){
			...
			$conn = Connection::getInstance();
			//chuan bi truy van
			$query = $conn->prepare("update slide set name=:var_name,description=:var_description,content=:var_content,hot=:var_hot where id=:var_id");
			//thuc thi truy van, co truyen tham so vao cau lenh sql
			...
			*/
			if($_FILES["photo"]["name"] != ""){
				...
				if($oldPhoto->rowCount() > 0){
					...
					//xoa anh
					if($record->photo != "" && file_exists("../assets/upload/slide/".$record->photo))
						unlink("../assets/upload/slide/".$record->photo);
				}
				//---
				$photo = time()."_".$_FILES["photo"]["name"];
				move_uploaded_file($_FILES["photo"]["tmp_name"],"../assets/upload/slide/$photo");
		...
		public function modelCreate(){
			...
			*/
			...
			//update name			
			//chuan bi truy van
			$query = $conn->prepare("insert into slide set name=:var_name,description=:var_description,content=:var_content,hot=:var_hot,photo=:var_photo");
			//thuc thi truy van, co truyen tham so vao cau lenh sql
		...
		public function modelDelete(){
			...
			//---
			//chuan bi truy van
			$query = $conn->prepare("delete from slide where id=:var_id or parent_id = $id");
			//thuc thi truy van, co truyen tham so vao cau lenh sql
			$query->execute(["var_id"=>$id]);
		}
	}
